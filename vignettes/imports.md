...
Falling Fruit aspires to be the world's most comprehensive foraging map. To this end, we seek out and import external datasets - many of which are vast and detailed municipal tree inventories containing hundreds of different species. We have developed a workflow to assist a human in preparing these data for import.
...
`fruitr::read_locations()` supports most delimited text files and all [OGR vector formats](http://www.gdal.org/ogr_formats.html) (provided you have the package [rgdal](https://cran.r-project.org/web/packages/rgdal/index.html) installed).
...
```R
ff_types <- fruitr::get_ff_types(key=API_KEY)[(!pending)]
